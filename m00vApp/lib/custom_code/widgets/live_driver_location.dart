...
class _LiveDriverLocationState extends State<LiveDriverLocation> {
  ...
  Future<void> _setCameraToRouteBounds(gmaps.LatLng currentLocation) async {
    final current = _currentPosition ?? widget.destination;
    try {
      final controller = await _mapController.future;
      final bounds = gmaps.LatLngBounds(
        southwest: gmaps.LatLng(
          min(currentLocation.latitude, widget.destination.latitude),
          min(currentLocation.longitude, widget.destination.longitude),
        ),
        northeast: gmaps.LatLng(
          max(currentLocation.latitude, widget.destination.latitude),
          max(currentLocation.longitude, widget.destination.longitude),
        ),
      );
    ...
    return SizedBox(
      ...
      child: gmaps.GoogleMap(
        ...
        markers: {
          ...
          if (_carIcon != null)
            ...widget.carPositions.asMap().entries.map(
              (entry) {
                final idx = entry.key;
                final pos = entry.value;
                return gmaps.Marker(
                  markerId: gmaps.MarkerId('car_$idx'),
                  position: _convertToGmapsLatLng(pos),
                  icon: _carIcon!,
                  zIndex: 1,
                );
              },
